...
parser.add_argument("cluster_list", type=str,
                    help="comma-separated list of Camoco clusters, or write "
                    "'all' to return correlations for entire network")
...
if cluster_list == "all":
    ...
    scores_df = COB.subnetwork(sig_only = True, names_as_cols = True)
    # keep only important columns
    scores_df = scores_df[["gene_a", "gene_b", "score"]]
    # export data frame data to tsv, GraphML and JSON formats
    scores_df.to_csv(output_prefix + ".txt", sep = "\t", index = False)
    COB.to_graphml(file = output_prefix + ".graphml")
    COB.to_json(filename = output_prefix + ".json", sig_only = True)
